...
function handleRequest(e) {
  ...
  switch (action) {
    ...
    default:
      return createJsonResponse(false, 'Acción no reconocida. Acciones disponibles: login, logout, cambiarPassword, listAuditoria, list, create, update, delete, createHorario, updateHorario, deleteHorario, listHorarios, getHorarioGrupo, generarSolicitudHorario');
  }
}
...
const HORARIOS_HEADERS = [
  ...
  'HORA_SALIDA', 'HORAS_DIA', 'OBSERVACION', 'HORAS_SEMANA'
];
...
function generarSolicitudHorario(params) {
  try {
    ...
    const blob = exportarSpreadsheet_(copiaFile.getId(), formato)
      .setName(`Solicitud_cambio_horario_${code}.${formato}`);
