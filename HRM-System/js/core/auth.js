...
window.AUTH = (function() {
    let sesion = null; // { token, usuario, nombre, rol, permisos, expiraEn }
    ...
    function limpiar() {
        sesion = null;
        sessionStorage.removeItem(AUTH_STORAGE_KEY);
        // Borra también el caché local de Personal (js/api.js). Sin esto,
        // si una sesión de admin dejó en localStorage la lista completa
        // (incluidos inactivos) y luego alguien más entra sin ser admin
        // en el mismo navegador, podría ver esos datos cacheados durante
        // hasta 5 minutos antes de que se refresquen desde el backend.
        localStorage.removeItem('staffHubData');
        localStorage.removeItem('staffHubTimestamp');
    }
    ...
    async function login(usuario, password) {
        ...
        if (result.success && result.data) {
            guardar({
                ...
                rol: result.data.rol,
                // El backend ya devuelve un objeto (no una cadena JSON) para
                // rol "usuario"; para "admin" puede venir vacío, ya que no
                // se usa (ver getRol()==='admin' más abajo).
    ...
    function getPermisos() { return sesion ? (sesion.permisos || {}) : {}; }

    // Comprobación de conveniencia para el frontend (ocultar/mostrar
    // menús y botones). Un "admin" siempre tiene acceso total. Esto es
    // SOLO comodidad visual: el backend vuelve a comprobar el mismo
    // permiso en cada petición (ver tienePermisoAccion_ en
    // Codigo_corregido.gs) sin confiar en lo que decida el navegador.
    function tienePermiso(modulo, accion) {
        if (!sesion) return false;
        if (sesion.rol === 'admin') return true;
        const permisosModulo = (sesion.permisos || {})[modulo];
        return !!(permisosModulo && permisosModulo[accion] === true);
    }
