...
window.HorarioExportXLSX = (function() {
    ...
    // ---- DETALLE PLANO (.xlsx): mismas 16 columnas que antes se
    // descargaban en CSV (una fila por día, TODOS los estados, sin
    // agrupar días) — usado por los botones "Exportar" (horarios de
    // un empleado) y "descargar horario individual" (un solo grupo),
    // que ya no generan CSV sino este mismo formato con estilo. ----
    const DETALLE_HEADERS = ['ID_GRUPO', 'FECHA_REGISTRO', 'FECHA_ACTUALIZACION', 'CODE', 'ID_PERSONAL', 'EMPLEADO', 'FECHA_INICIO', 'FECHA_FIN', 'DIA', 'HORA_INGRESO', 'INICIO_REFRIGERIO', 'FIN_REFRIGERIO', 'HORA_SALIDA', 'HORAS_DIA', 'HORAS_SEMANA', 'OBSERVACION'];
    const DETALLE_ANCHOS = [12, 15, 16, 10, 12, 26, 12, 12, 12, 11, 14, 12, 11, 10, 12, 22];

    function generarDetalle(grupos, tituloArchivo, tituloVisible) {
        if (typeof XLSX === 'undefined' || !XLSX.utils) {
            window.toast('⚠️ No se pudo cargar la librería de Excel (XLSX). Revisa tu conexión.', 'warning');
            return false;
        }
        ...
        const filas = window.HorarioExport.aFilas(grupos);
        ...
        filas.forEach((fila, idx) => {
            ...
            fila.forEach((valor, c) => {
                setCell(r, c, valor, {
                    ...
                    border: bordeFino(),
                    fill: { fgColor: { rgb: banda } },
                    alignment: { horizontal: c === 5 ? 'left' : 'center', vertical: 'center', wrapText: c === 15 }
                });
            });
            r++;
        });
