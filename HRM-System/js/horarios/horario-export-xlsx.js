...
window.HorarioExportXLSX = (function() {
    ...
    function construirFilas(gruposVigentes) {
        return gruposVigentes.map(grupo => {
            ...
            return {
                ...
                fechaInicioRaw: String(grupo.FECHA_INICIO || '').slice(0, 10),
    ...
    function generar(gruposVigentes) {
        ...
        // Agrupa por colaborador (mismo criterio que el macro: nombre
        // de A a Z) y, dentro de un mismo colaborador, ordena sus
        // horarios por FECHA INICIO (columna B) de la fecha más
        // antigua a la más reciente — mismo criterio de fecha que
        // usa ordenarGruposPorFechaInicio() en el resto del sistema
        // (panel "Horarios registrados" del modal y export DETALLE).
        const filas = construirFilas(gruposVigentes).sort((a, b) => {
            const porColaborador = a.colaboradorId.localeCompare(b.colaboradorId, 'es');
            if (porColaborador !== 0) return porColaborador;
            if (a.fechaInicioRaw < b.fechaInicioRaw) return -1;
            if (a.fechaInicioRaw > b.fechaInicioRaw) return 1;
            return 0;
        });
