...
window.Horarios.calcularResumenHoras = function() {
    ...
    document.getElementById('promedioHorasDia').textContent = window.Horarios.formatHoras(promedio);
};

window.Horarios.actualizarContadorDiasSeleccionados = function() {
    const count = DIAS_SEMANA.filter(d => horariosPorDia[d].activo).length;
    const el = document.getElementById('diasSeleccionadosCount');
    if (el) el.textContent = `(${count} día${count !== 1 ? 's' : ''} seleccionado${count !== 1 ? 's' : ''})`;
};
...
window.Horarios.actualizarVistaPrevia = function() {
    ...
    if (diasActivos.length === 0) {
        container.innerHTML = `<span style="color:#94A3B8;font-size:12px;">Selecciona días y completa los horarios para ver la vista previa</span>`;
        return;
    }
    ...
    DIAS_SEMANA.forEach(dia => {
        ...
        if (!data.entrada || !data.salida || !data.inicioRefrigerio || !data.terminoRefrigerio) {
            html += `
                <div class="dia-line inactivo">
                    <span class="dia-nombre">${dia}</span>
                    <span style="color:#94A3B8;font-size:10px;">⚠️ Incompleto</span>
                </div>
            `;
            return;
        }
        ...
        html += `
            <div class="dia-line">
                ...
                ${horas > 0 ? `<span class="total-horas">${window.Horarios.formatHoras(horas)}h</span>` : ''}
            </div>
        `;
    });

    if (!html) {
        container.innerHTML = `<span style="color:#94A3B8;font-size:12px;">Completa los horarios para ver la vista previa</span>`;
    } else {
...
window.Horarios.guardar = async function(e) {
    ...
    for (const dia of diasActivos) {
        ...
        horarios.push({
            ...
            nombre: document.getElementById('nombrePersonalDisplay').value.trim() || 'Sin nombre',
    ...
    try {
        if (isEdit) {
            ...
            if (todosExitosos) {
                ...
                window.toast(`✅ Horario(s) actualizado(s)`, 'success');
...
window.Horarios.guardarEnSheet = async function(horarioData) {
    try {
        ...
        let params = '';
        Object.entries(horarioData).forEach(([k, v]) => {
