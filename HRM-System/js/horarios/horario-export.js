...
window.HorarioExport = (function() {
    const CSV_HEADERS = ['ID_GRUPO', 'EMPLEADO', 'ID_PERSONAL', 'CODE', 'ESTADO', 'FECHA_INICIO', 'FECHA_FIN', 'DIA', 'INGRESO', 'INICIO_REFRIGERIO', 'FIN_REFRIGERIO', 'SALIDA', 'HORAS_DIA', 'HORAS_SEMANA', 'OBSERVACION'];

    // Aplana uno o varios grupos (cabecera + DIAS anidados) a filas
    // planas, una por día, listas para CSV — el mismo formato "una
    // fila por día" que ya usa BD_HORARIOS en la hoja original.
    // OBSERVACION va siempre AL FINAL (última columna), para que no
    // quede intercalada entre las columnas de horas.
    function aFilas(grupos) {
        const filas = [];
        grupos.forEach(g => {
            (g.DIAS || []).forEach(d => {
                filas.push([
                    g.ID_GRUPO, g.EMPLEADO, g.ID_PERSONAL, g.CODE, g.ESTADO,
...
// Botón del sidebar: "Exportar Horarios" — descarga el CSV con TODOS
// los horarios de TODOS los empleados (no requiere tener a nadie
// seleccionado en Lista de Personal).
window.exportarTodosHorarios = function() {
