...
// descargar el horario completo de UN grupo seleccionado (.xlsx), (b)
...
// (función de respaldo, ya no usada desde ningún botón activo).
// ============================================================
...
window.HorarioExport = (function() {
    ...
    // gruposDelEmpleado (opcional): el resto de horarios de este mismo
    // empleado, ya cargados en el panel del modal — se usan solo para
    // calcular la FECHA_FIN encadenada de `grupo` en función del que le
    // sigue cronológicamente, aunque se descargue un único horario.
    function descargarGrupo(grupo, gruposDelEmpleado) {
        const nombreBase = `horario_${grupo.ID_GRUPO}_${(grupo.EMPLEADO || '').replace(/[^\wÀ-ÿ]+/g, '_')}`;
        const ok = window.HorarioExportXLSX.generarDetalle([grupo], nombreBase, `DETALLE DE HORARIO — ${grupo.EMPLEADO || ''}`, gruposDelEmpleado);
        if (ok) window.toast('📥 Horario descargado', 'success');
    }
    ...
    async function descargarDeEmpleado(code, tituloArchivo, tituloVisible) {
        const result = await window.HorarioAPI.listar(code);
        if (!result.success || !Array.isArray(result.data) || result.data.length === 0) {
            window.toast('⚠️ Este empleado no tiene horarios registrados', 'warning');
            return;
        }
        const ok = window.HorarioExportXLSX.generarDetalle(result.data, tituloArchivo, tituloVisible);
        if (ok) window.toast(`📥 ${result.data.length} horario(s) descargado(s)`, 'success');
    }

    return { aFilas, aCSV, aTextoLegible, descargarGrupo, copiarGrupo, descargarTodos, descargarDeEmpleado };
})();
...
// horarios del empleado actualmente seleccionado, en .xlsx.
window.exportarHorariosEmpleadoActual = function() {
    ...
    const nombreBase = `horarios_${window.personalSeleccionado.CODE}_${(window.personalSeleccionado.NOMBRES || '').replace(/[^\wÀ-ÿ]+/g, '_')}`;
    window.HorarioExport.descargarDeEmpleado(window.personalSeleccionado.CODE, nombreBase, `DETALLE DE HORARIOS — ${window.personalSeleccionado.NOMBRES || ''}`);
};
