// ============================================================
// MÓDULO: HORARIOS - VERSIÓN JSON LOCAL
// ============================================================
...
window.horarios = (function() {
    ...
    // ===== CONFIGURACIÓN =====
    const HORARIOS_JSON_PATH = 'data/horarios.json';
    const STORAGE_KEY_HORARIOS = 'horariosData';
    const STORAGE_TIMESTAMP_KEY_HORARIOS = 'horariosTimestamp';
    ...
    // ===== CARGAR PERSONAL DESDE API LOCAL =====
    async function cargarPersonal() {
        try {
            // Usar window.API para cargar personal
            const data = await window.API.list(true);
            datosPersonal = data || [];
        } catch (e) {
            console.error(e);
    ...
    async function cargarHorarios(force = false) {
        try {
            if (!force) {
                const cached = localStorage.getItem(STORAGE_KEY_HORARIOS);
                const ts = localStorage.getItem(STORAGE_TIMESTAMP_KEY_HORARIOS);
            ...
            const resp = await fetch(HORARIOS_JSON_PATH + '?' + Date.now());
            if (!resp.ok) {
                throw new Error('No se pudo cargar ' + HORARIOS_JSON_PATH);
            }
            horariosData = await resp.json();
            
            if (!Array.isArray(horariosData)) {
                throw new Error('El archivo horarios.json no contiene un array válido');
            }
            
            localStorage.setItem(STORAGE_KEY_HORARIOS, JSON.stringify(horariosData));
            localStorage.setItem(STORAGE_TIMESTAMP_KEY_HORARIOS, String(Date.now()));
            return horariosData;
        } catch (e) {
            console.error('Error al cargar horarios.json:', e);
            const fallback = localStorage.getItem(STORAGE_KEY_HORARIOS);
            if (fallback) {
                try {
                    horariosData = JSON.parse(fallback);
                    toast('⚠️ Usando caché de horarios', 'warning');
                    return horariosData;
                } catch (e2) {}
            }
            toast('❌ Error al cargar horarios: ' + e.message, 'error');
            return [];
        }
    }

    // ===== GUARDAR HORARIOS EN JSON =====
    function guardarHorariosJSON(data) {
        ...
        toast('📁 Reemplaza el archivo en data/horarios.json', 'info');
    }
    ...
    async function cargarHorariosEmpleado(idPersonal) {
        try {
            ...
            const horariosEmpleado = horariosData.filter(h => String(h.idPersonal) === String(idPersonal));

            if (horariosEmpleado.length > 0) {
                const ultimo = horariosEmpleado[horariosEmpleado.length - 1];
                ...
                calcularResumen();
                toast(`📋 ${horariosEmpleado.length} horario(s) cargado(s)`, 'info');
            } else {
    ...
    async function guardarHorario() {
        ...
        try {
            ...
            for (const horario of horarios) {
                const nuevoHorario = {
                    ...horario,
                    totalHoras: formatHoras(calcularHorasDia(
                        horario.horaEntrada,
                        horario.inicioRefrigerio,
                        horario.terminoRefrigerio,
                        horario.horaSalida
                    )),
                    tipo: tipo,
                    _rowIndex: horariosData.length + 1
                };
                horariosData.push(nuevoHorario);
            }
            ...
            // Descargar JSON
            guardarHorariosJSON(horariosData);
    ...
    function init() {
        ...
        console.log('✅ Módulo Horarios inicializado (JSON local)');
    }
...
document.addEventListener('DOMContentLoaded', function() {
    if (window.horarios && window.horarios.init) {
        setTimeout(() => window.horarios.init(), 200);
    }
});