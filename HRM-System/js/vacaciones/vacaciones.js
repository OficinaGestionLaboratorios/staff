...
window.vacacionVerCompleto = async function(registro) {
    ...
    precargarCamposDocumento(result.data);
    await window.vacacionesCargarLista();
};
...
window.guardarVacacionFase1 = async function() {
    ...
    try {
        ...
        if (result.success) {
            ...
            const idEditado = vacacionIdEnEdicion;
            vacacionIdEnEdicion = null;
            window.VacacionesUI.limpiarFase1();
            window.VacacionesUI.setModoFase1(false);
            await window.vacacionesCargarLista();
            // Si el período que se acaba de editar es el mismo que está
            // abierto en Fase 2 (el usuario corrigió DNI/ID_PERSONAL sin
            // cerrar el modal), hay que refrescar también
            // vacacionRegistroFase2Actual — si no, "Generar documento"
            // sigue leyendo el objeto viejo en memoria (con el DNI/ID en
            // blanco) hasta que se cierre y reabra el modal.
            if (idEditado && idEditado === vacacionIdParaGoce) {
...
// Prellena los inputs DNI/ID Personal del resumen de Fase 2. Prioridad:
// 1) lo que ya haya en el propio input (si el usuario lo tocó, no se
//    pisa); 2) el propio registro de VACACIONES; 3) fallback por CODE
// contra Personal (personalSeleccionado o la caché de window.API).
function precargarCamposDocumento(registro) {
    const personal = vacacionBuscarPersonalPorCode(registro.CODE);
    const inputDni = document.getElementById('vacacionDniDocumento');
    const inputId = document.getElementById('vacacionIdPersonalDocumento');
    if (inputDni && !inputDni.value) {
        inputDni.value = registro.DNI || (personal ? personal.DNI : '') || '';
    }
    if (inputId && !inputId.value) {
        inputId.value = registro.ID_PERSONAL || (personal ? personal.ID_PERSONAL : '') || '';
    }
}
...
async function vacacionGenerarDocumentoDeGoce(registro, indice) {
    ...
    // DNI e ID_PERSONAL se toman PRIMERO de los inputs editables del
    // resumen de Fase 2 (#vacacionDniDocumento/#vacacionIdPersonalDocumento):
    // esos campos son la fuente que el usuario ve y puede corregir a
    // mano antes de generar el documento, así que no dependen de que
    // el registro guardado en BD_VACACIONES o la ficha de Personal
    // estén sincronizados. Si por alguna razón el input no existe en
    // el DOM en ese momento, cae al mismo cálculo de siempre como
    // respaldo.
    const inputDni = document.getElementById('vacacionDniDocumento');
    const inputId = document.getElementById('vacacionIdPersonalDocumento');
    const dni = (inputDni ? inputDni.value.trim() : '') || registro.DNI || (personal ? personal.DNI : '') || '';
    const idPersonal = (inputId ? inputId.value.trim() : '') || registro.ID_PERSONAL || (personal ? personal.ID_PERSONAL : '') || '';

    if (!dni) {
        window.toast('⚠️ Falta el DNI: complétalo en el recuadro de arriba antes de generar el documento.', 'warning');
        return;
    }
...
window.vacacionRecargarFase2 = async function() {
    ...
    // mostrarFormFase2 reconstruye todo el HTML del resumen (incluidos
    // los inputs DNI/ID Personal), así que si no se guardan sus
    // valores actuales antes de repintar, cualquier corrección que el
    // usuario haya tecleado a mano se perdería al registrar un nuevo
    // tramo.
    const dniPrevio = document.getElementById('vacacionDniDocumento')?.value.trim() || '';
    const idPrevio = document.getElementById('vacacionIdPersonalDocumento')?.value.trim() || '';
    ...
    if (actualizado.success && actualizado.data) {
        ...
        if (dniPrevio) document.getElementById('vacacionDniDocumento').value = dniPrevio;
        if (idPrevio) document.getElementById('vacacionIdPersonalDocumento').value = idPrevio;
        precargarCamposDocumento(actualizado.data);
    }
};
