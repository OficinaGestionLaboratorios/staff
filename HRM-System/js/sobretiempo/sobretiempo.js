...
// Control de Asistencia confirma que el descanso ya se tomó: marca
// esa solicitud DC-xxxxxx como COMPLETADO y libera sus horas del
// banco (pasan de "comprometidas" a "utilizadas").
window.sobretiempoCompletarDescansoBanco = async function(idSolicitudDescanso) {
    if (!idSolicitudDescanso) return;
    const confirmado = confirm(`¿Confirmas que el descanso de la solicitud ${idSolicitudDescanso} ya fue tomado?\n\nSe marcará como COMPLETADO y sus horas quedarán como utilizadas en el banco de horas.`);
    if (!confirmado) return;
    try {
        const result = await window.SobretiempoAPI.completarSolicitudBanco(idSolicitudDescanso);
        if (result.success) {
            window.toast(`✅ ${idSolicitudDescanso} marcada como completada`, 'success');
            if (window.personalSeleccionado) await window.sobretiempoAbrirFase2();
        } else {
            window.toast('❌ ' + (result.message || 'No se pudo actualizar la solicitud'), 'error');
        }
    } catch (e) {
        window.toast('❌ Error al actualizar la solicitud', 'error');
    }
};
