...
window.SobretiempoModel = (function() {
    ...
    const ESTADO_INFO = {
        'PENDIENTE':              { clase: 'estado-programado', icono: 'fa-hourglass-half' },
        'PARCIALMENTE UTILIZADO': { clase: 'estado-parcial',    icono: 'fa-adjust' },
        'COMPLETADO':              { clase: 'estado-vigente',    icono: 'fa-check-circle' },
        // Compatibilidad con la Fase 1 existente; no son estados del Banco de horas.
    ...
    function construirPayloadBanco({ personal, selecciones, fechasDescanso, equivalencia, observaciones }) {
        return {
            CODE: personal.CODE,
            ID_PERSONAL: personal.ID_PERSONAL || '',
            EMPLEADO: window.formatearPersonalSeleccionado(personal),
            SELECCIONES_JSON: JSON.stringify(selecciones || []),
            FECHAS_DESCANSO_JSON: JSON.stringify(fechasDescanso || []),
            EQUIVALENCIA: equivalencia || '',
            OBSERVACIONES: (observaciones || '').trim()
        };
    }

    function calcularEquivalenciaBanco(totalHoras, jornadaDiaria) {
        const total=parseFloat(totalHoras)||0, jornada=parseFloat(jornadaDiaria)||0;
        if (!jornada || jornada<=0) return `${total.toFixed(2)} h`;
        const dias=Math.floor((total+0.0001)/jornada);
        const resto=+(total-dias*jornada).toFixed(2);
        if (dias && resto) return `${dias} día${dias===1?'':'s'} + ${resto.toFixed(2)} h`;
        if (dias) return `${dias} día${dias===1?'':'s'}`;
        return `${total.toFixed(2)} h`;
    }

    // ---- Helpers para el correo de descanso compensatorio (Fase 2) ----
    const DIAS_SEMANA_ES = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'];

    // "domingo, 28/06/2026" — mismo formato que usa el modelo de
    // correo real (día de la semana en minúscula + fecha dd/mm/yyyy).
    function diaSemanaYFechaTexto(fechaISO) {
        if (!fechaISO) return '';
        const soloFecha = String(fechaISO).slice(0, 10);
        const [y, m, d] = soloFecha.split('-').map(Number);
        if (!y || !m || !d) return fechaATextoLegible(fechaISO);
        const fecha = new Date(y, m - 1, d);
        const dia = DIAS_SEMANA_ES[fecha.getDay()] || '';
        return `${dia}, ${fechaATextoLegible(fechaISO)}`;
    }

    // 8.8 -> "8:48" (mismo formato "TOTAL" que usa el modelo de correo
    // real, horas:minutos en vez de horas decimales).
    function horasDecimalATextoHM(horasDecimal) {
        const horas = parseFloat(horasDecimal) || 0;
        const totalMin = Math.round(horas * 60);
        const hh = Math.floor(Math.abs(totalMin) / 60);
        const mm = Math.abs(totalMin) % 60;
        return `${totalMin < 0 ? '-' : ''}${hh}:${String(mm).padStart(2, '0')}`;
    }
    ...
    return {
        ...
        construirPayloadFase1, construirPayloadFase2, construirPayloadBanco, calcularEquivalenciaBanco,
        diaSemanaYFechaTexto, horasDecimalATextoHM
    };
})();
