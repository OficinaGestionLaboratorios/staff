...
window.SobretiempoAPI = (function() {
    ...
    async function obtenerBanco(code) {
        const url = window.API_URL + '?action=obtenerBancoHoras&code=' + encodeURIComponent(code || '');
        return llamar(url);
    }

    async function crearSolicitudBanco(payload) {
        const url = window.API_URL + '?action=crearSolicitudDescansoBanco&key=' + encodeURIComponent(window.API_KEY) + construirQuery(payload);
        return llamar(url);
    }

    async function listarSolicitudesBanco(code) {
        const url = window.API_URL + '?action=listarSolicitudesDescansoBanco' + (code ? '&code=' + encodeURIComponent(code) : '');
        return llamar(url);
    }

    async function anularSolicitudBanco(idSolicitudDescanso) {
        const url = window.API_URL + '?action=anularSolicitudDescansoBanco&key=' + encodeURIComponent(window.API_KEY) + '&idSolicitudDescanso=' + encodeURIComponent(idSolicitudDescanso || '');
        return llamar(url);
    }

    async function registrarDescansoEfectuadoBanco(payload) {
        const url = window.API_URL + '?action=registrarDescansoEfectuadoBanco&key=' + encodeURIComponent(window.API_KEY) + construirQuery(payload);
        return llamar(url);
    }

    return { listar, obtener, crear, actualizar, registrarDescanso, eliminarDescanso, eliminar, obtenerBanco, crearSolicitudBanco, listarSolicitudesBanco, anularSolicitudBanco, registrarDescansoEfectuadoBanco };
})();
