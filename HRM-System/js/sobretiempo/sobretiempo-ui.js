...
window.SobretiempoUI = (function() {
            ...
            return `
                <div class="horario-grupo-item ${activo ? 'activo' : ''}" data-id-solicitud="${window.esc(r.ID_SOLICITUD)}">
                    ...
                    <div class="horario-grupo-acciones">
                        ${pendiente ? `
                            <button type="button" class="btn-chip btn-chip-green" data-accion="registrar-descanso" data-id="${window.esc(r.ID_SOLICITUD)}"><i class="fas fa-piggy-bank"></i> Usar horas del banco</button>
    ...
    function renderBancoHoras(banco, solicitudes, jornadaDiaria) {
        ...
        if(res){
            res.innerHTML=(solicitudes||[]).slice(0,5).map(x=>`<div style="font-size:12px;color:#64748B;padding:5px 0;border-top:1px solid #E2E8F0;"><strong>${window.esc(x.ID_SOLICITUD_DESCANSO)}</strong> · ${window.esc(x.TOTAL_HORAS)} h · ${window.esc(x.ESTADO)}</div>`).join('');
        }
    }
