...
window.SobretiempoUI = (function() {
    ...
    function renderLista(registros, idSolicitudActiva) {
        ...
        cont.innerHTML = registros.map(r => {
            ...
            // "Editar" solo tiene sentido si la solicitud no tiene
            // descansos ni consumos reales registrados en el banco.
            // Las solicitudes pendientes del banco se controlan en backend.
    ...
    function actualizarTotalFilaDescanso(fila) {
        ...
        if (!span) return;
        span.textContent = (ini && fin) ? M.calcularTotalHoras(ini, fin, '', '') + ' h' : '—';
    }
    ...
    function inicializarBancoHoras() {
        ...
        if(fechas && !fechas.dataset.init){
            ...
            fechas.addEventListener('input',e=>{ if(e.target.matches('[data-banco-fecha-descanso-hora-inicio],[data-banco-fecha-descanso-hora-fin]')) actualizarTotalFilaDescanso(e.target.closest('div')); });
    ...
    function renderBancoHoras(banco, solicitudes, jornadaDiaria) {
        ...
        body.innerHTML=disponibles.length ? disponibles.map((x,i)=>`<tr>
            ...
            <td data-fecha-banco="${window.esc(x.FECHA_SOBRETIEMPO)}" data-hora-inicio-banco="${window.esc(x.HORA_INICIO||'')}" data-hora-fin-banco="${window.esc(x.HORA_FIN||'')}" data-id-solicitud-banco="${window.esc(x.ID_SOLICITUD_ORIGEN||'')}">${window.esc(M.fechaATextoLegible(x.FECHA_SOBRETIEMPO))}</td>
        ...
        </tr>`).join('') : '<tr><td colspan="7" style="text-align:center;color:#64748B;padding:18px;">No hay horas disponibles para utilizar.</td></tr>';
        ...
        if(res){
            ...
            res.innerHTML=recientes.length ? recientes.map(x=>`<div style="font-size:12px;color:#64748B;padding:8px 0;border-top:1px solid #E2E8F0;display:flex;justify-content:space-between;gap:10px;align-items:center;">
                <span><strong>${window.esc(x.TOTAL_HORAS)} h</strong> · ${window.esc(x.ESTADO)}${parseFloat(x.HORAS_UTILIZADAS||0)>0 ? ` · ${window.esc(x.HORAS_UTILIZADAS)} h utilizadas · ${window.esc(x.HORAS_PENDIENTES)} h pendientes` : ''}</span>
                <span style="display:flex;gap:6px;align-items:center;">
                    ${(x.ESTADO==='PENDIENTE' || x.ESTADO==='PARCIALMENTE UTILIZADO') ? `<button type="button" class="btn-chip btn-chip-blue" data-accion="registrar-descanso-efectuado" data-id="${window.esc(x.ID_SOLICITUD_DESCANSO)}" data-total="${window.esc(x.TOTAL_HORAS)}" data-pendiente="${window.esc(x.HORAS_PENDIENTES || x.TOTAL_HORAS)}" style="padding:3px 8px;font-size:11px;"><i class="fas fa-check-circle"></i> ${x.ESTADO==='PARCIALMENTE UTILIZADO' ? 'Registrar descanso restante' : 'Registrar descanso efectuado'}</button>` : ''}
                </span>
            </div>`).join('') : '<div style="font-size:12px;color:#94A3B8;padding:5px 0;">No hay solicitudes recientes.</div>';
        }
    }
    ...
    function leerBancoHoras() {
        ...
        document.querySelectorAll('#sobretiempoBancoBody tr').forEach(tr=>{
            ...
            if(h<=0)return;
            // horaInicio/horaFin viajan solo para el correo. El backend
            // utiliza origen+fecha+horas para validar la solicitud.
            // La columna "Solicitud origen" ya no se muestra en la
            // tabla, pero el ID se sigue necesitando: viaja oculto
            // como data-id-solicitud-banco en la celda de fecha.
            selecciones.push({
                ID_SOLICITUD_ORIGEN:d[1]?.dataset?.idSolicitudBanco||'',
        ...
        const fechasDescanso=Array.from(document.querySelectorAll('#sobretiempoFechasDescansoBody > div')).map(div=>({
            ...
            ...f,
            horas: (f.horaInicio && f.horaFin) ? M.calcularTotalHoras(f.horaInicio, f.horaFin, '', '') : ''
        }));
