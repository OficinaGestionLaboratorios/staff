...
window.SobretiempoUI = (function() {
    ...
    function leerFase1() {
        ...
        return {
            ...
            refrigerioInicio: document.getElementById('sobretiempoRefrigerioInicio')?.value || '',
            refrigerioFin: document.getElementById('sobretiempoRefrigerioFin')?.value || '',
    ...
    function pintarFase1(registro) {
        ...
        document.getElementById('sobretiempoRefrigerioInicio').value = registro.REFRIGERIO_INICIO || '';
        document.getElementById('sobretiempoRefrigerioFin').value = registro.REFRIGERIO_FIN || '';
    ...
    function limpiarFase1() {
        ...
        ['sobretiempoDependencia', 'sobretiempoFechaEjecucion', 'sobretiempoHoraInicio', 'sobretiempoHoraFin',
         'sobretiempoRefrigerioInicio', 'sobretiempoRefrigerioFin', 'sobretiempoActividades', 'sobretiempoJustificacion'].forEach(id => {
    ...
    // Recalcula "horas efectivas generadas" (jornada − refrigerio) y,
    // aparte, muestra cuánto se está descontando por refrigerio.
    function actualizarTotalHoras() {
        const { horaInicio, horaFin, refrigerioInicio, refrigerioFin } = leerFase1();
        const total = document.getElementById('sobretiempoTotalHoras');
        if (total) total.textContent = M.calcularTotalHoras(horaInicio, horaFin, refrigerioInicio, refrigerioFin) + ' h';
        const refrigerio = document.getElementById('sobretiempoRefrigerioDuracion');
        if (refrigerio) refrigerio.textContent = (refrigerioInicio && refrigerioFin) ? '−' + M.calcularDuracionRefrigerio(refrigerioInicio, refrigerioFin) + ' h de refrigerio' : 'Sin refrigerio registrado';
    }
    ...
    function mostrarFormFase2(registro) {
        ...
        const tieneRefrigerio = registro.REFRIGERIO_INICIO && registro.REFRIGERIO_FIN;
        document.getElementById('sobretiempoFase2Resumen').innerHTML = `
            <div>
                ...
                <span style="${chipStyle}"><i class="fas fa-clock"></i> ${window.esc(registro.HORA_INICIO)} a ${window.esc(registro.HORA_FIN)} (${window.esc(registro.TOTAL_HORAS)} h efectivas)</span>
                ${tieneRefrigerio ? `<span style="${chipStyle}"><i class="fas fa-mug-hot"></i> Refrigerio: ${window.esc(registro.REFRIGERIO_INICIO)} a ${window.esc(registro.REFRIGERIO_FIN)}</span>` : ''}
            </div>
        ...
        // Se sugiere el mismo horario de la jornada que generó las
        // horas como punto de partida; Control de Asistencia lo ajusta
        // al horario real en que se tomó el descanso.
        document.getElementById('sobretiempoHoraInicioDescanso').value = registro.HORA_INICIO || '';
        document.getElementById('sobretiempoHoraFinDescanso').value = registro.HORA_FIN || '';
