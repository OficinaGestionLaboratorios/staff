...
function instalarListenersGlobales() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (modalUsuarios && modalUsuarios.classList.contains('active') && e.target === modalUsuarios) {
            window.cerrarModalUsuarios?.();
        }
...
window.iniciarApp = function() {
    ...
    // El ítem "Usuarios" del sidebar solo se muestra a sesiones con
    // ROL = "admin" (comodidad visual: el backend vuelve a exigir
    // ese rol en listUsuarios/crearUsuario/actualizarUsuario, ver
    // requerirAdmin_ en Codigo_corregido.gs).
    const menuUsuarios = document.getElementById('menuItemUsuarios');
    if (menuUsuarios) menuUsuarios.style.display = (window.AUTH.getRol() === 'admin') ? '' : 'none';

    // Resto de ítems del sidebar: para un "admin" quedan todos
    // visibles; para un "usuario" normal, cada módulo se oculta si su
    // permiso "ver" (asignado en Usuarios, ver permisos-config.js) no
    // está activo. Comodidad visual — el backend vuelve a exigir el
    // mismo permiso en cada petición (tienePermisoAccion_).
    if (window.MODULO_A_MENU_ID) {
        Object.keys(window.MODULO_A_MENU_ID).forEach(modulo => {
            const idBoton = window.MODULO_A_MENU_ID[modulo];
            if (!idBoton) return;
            const boton = document.getElementById(idBoton);
            if (!boton) return;
            boton.style.display = window.AUTH.tienePermiso(modulo, 'ver') ? '' : 'none';
        });
    }
