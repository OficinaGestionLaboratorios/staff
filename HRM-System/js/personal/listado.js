...
// Recuerda si el usuario decidió ver también los inactivos.
// Persiste en localStorage (no sessionStorage) a propósito: el
// usuario pidió que esta preferencia sobreviva al cerrar sesión y
// volver a entrar, no solo un cambio de vista dentro de la misma
// sesión.
//
// Un paso más allá: ver inactivos es exclusivo de ROL=admin. Ya no
// es solo que el checkbox quede oculto para una cuenta normal (eso
// sería solo comodidad visual) — el backend (getEmployees en
// Codigo_corregido.gs) directamente no envía esos registros a nadie
// que no sea admin, así que tampoco quedan expuestos en el caché
// local ni se pueden leer desde la consola del navegador.
window.mostrarInactivos = window.AUTH.getRol() === 'admin' && localStorage.getItem('mostrarInactivos') === '1';
...
window.renderListado = async function() {
    ...
    main.innerHTML = `
        ...
        <div class="action-bar">
            ...
            ${window.AUTH.getRol() === 'admin' ? `
            <label class="check-mostrar-inactivos" title="Ver inactivos es exclusivo de administrador">
    ...
    if (chkInactivos) {
        chkInactivos.addEventListener('change', function() {
            ...
            localStorage.setItem('mostrarInactivos', this.checked ? '1' : '0');
