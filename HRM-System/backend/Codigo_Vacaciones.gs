...
// estado inconsistente. El frontend ya oculta el botón "Editar" en
// ese caso (ver VacacionesUI.renderLista/sinGoces), pero el backend
// lo vuelve a exigir por si acaso.
function updateVacacion(params) {
  try {
    ...
    if (goces.length > 0) {
      return createJsonResponse(false, 'No se puede editar un período que ya tiene goces registrados. Elimínalo y crea uno nuevo si necesitas corregirlo.');
    }
