...
function crearUsuario(params) {
  try {
    ...
    const sheet = getUsersSheet();
    sheet.appendRow([usuario, salt, hashPassword(password, salt), nombre, rol, activo, new Date(), '']);

    registrarAuditoria(chk.sesion.usuario, 'CREAR_USUARIO', 'Usuarios',
      'Creó el usuario "' + usuario + '" (' + nombre + ') con rol ' + rol, usuario);

    return createJsonResponse(true, 'Usuario creado correctamente', { usuario: usuario, nombre: nombre, rol: rol, activo: activo });
...
function actualizarUsuario(params) {
  try {
    ...
    registrarAuditoria(chk.sesion.usuario, 'ACTUALIZAR_USUARIO', 'Usuarios',
      'Actualizó el usuario "' + usuario + '"' + (passwordNueva !== null ? ' (incluye restablecer contraseña)' : ''), usuario);
