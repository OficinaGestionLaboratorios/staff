...
// VACACIONES  -> hoja "BD_VACACIONES"  (ver Codigo_Vacaciones.gs)
// Son hojas completamente independientes: un error en la lógica de
// un módulo nunca puede corromper el CRUD de los demás.
// ============================================================
...
// Módulo Vacaciones agregado al final de la lista: createVacacion /
// updateVacacion / deleteVacacion modifican BD_VACACIONES, y
// registrarGoceVacacion / eliminarGoceVacacion modifican el arreglo
// de goces embebido dentro de esa misma fila — las cuatro son
// escrituras y por tanto exigen la API key igual que el resto.
const WRITE_ACTIONS = ['create', 'update', 'delete', 'createHorario', 'updateHorario', 'deleteHorario',
  'createSobretiempo', 'updateSobretiempo', 'registrarDescansoSobretiempo', 'deleteSobretiempo','eliminarDescansoSobretiempo','crearSolicitudDescansoBanco','completarDescansoBanco','createPermiso', 'updatePermiso', 'deletePermiso',
  'createVacacion', 'updateVacacion', 'deleteVacacion', 'registrarGoceVacacion', 'eliminarGoceVacacion', 'createLicencia',
  // PORTAL PERSONAL: el propio trabajador guardando sus datos también
  // pasa por la API_KEY, igual que el resto de escrituras.
  'actualizarMisDatos',
  // Configuración de campos del Portal de Autoactualización (ícono de
  // ajustes en Gestión Personal): cambia qué puede editar/ver el
  // trabajador, así que se trata como escritura sensible.
  'setConfigCamposPortal',
  // Gestión de Usuarios (crear/editar/activar-desactivar cuentas de
  // BD_USUARIOS): además de exigir sesión con ROL = "admin" (ver
  // requerirAdmin_ más abajo), pasan por la API key como cualquier
  // otra escritura sensible.
...
// PERMISOS: columna nueva con un JSON de la forma
// {"personal":{"ver":true,"crear":false,...}, "horarios":{...}, ...}
// (ver PERMISOS_MODULOS más abajo para el catálogo completo de
// módulos/acciones — debe reflejar exactamente
// js/usuarios/permisos-config.js del frontend). Solo tiene efecto
// para ROL = "usuario": un "admin" siempre tiene acceso total,
// ver tienePermisoAccion_.
const USERS_HEADERS = ['USUARIO', 'SALT', 'PASSWORD_HASH', 'NOMBRE', 'ROL', 'ACTIVO', 'FECHA_CREACION', 'ULTIMO_ACCESO', 'PERMISOS'];
const AUDIT_HEADERS = ['FECHA', 'USUARIO', 'ACCION', 'MODULO', 'DETALLE', 'ID_AFECTADO'];

// ============================================================
// PERMISOS DE ACCESO GRANULARES (checkboxes por módulo/acción)
// ============================================================
// Catálogo de módulos y, para cada uno, qué acciones existen.
// MISMA estructura que window.MODULOS_PERMISOS en
// js/usuarios/permisos-config.js — si agregas un módulo o una
// acción nueva, actualiza los dos lados.
const PERMISOS_MODULOS = {
  personal: ['ver', 'crear', 'editar', 'eliminar', 'exportar'],
  horarios: ['ver', 'crear', 'editar', 'eliminar', 'exportar'],
  sobretiempo: ['ver', 'crear', 'editar', 'eliminar', 'exportar'],
  vacaciones: ['ver', 'crear', 'editar', 'exportar'],
  permisos: ['ver', 'crear', 'editar', 'eliminar'],
  licencias: ['ver', 'crear'],
  auditoria: ['ver'],
  portalPersonal: ['ver', 'editar']
};

// Mapa acción del backend (handleRequest) -> {modulo, accion} que
// exige. Las acciones que NO aparecen aquí (login, logout,
// cambiarPassword, listUsuarios/crearUsuario/actualizarUsuario —
// estas ya se protegen aparte con requerirAdmin_ — y las del Portal
// de Autoactualización, que usan su propia sesión) no pasan por esta
// comprobación de permisos granulares.
const ACCIONES_MODULO = {
  list: { modulo: 'personal', accion: 'ver' },
  create: { modulo: 'personal', accion: 'crear' },
  update: { modulo: 'personal', accion: 'editar' },
  delete: { modulo: 'personal', accion: 'eliminar' },
  listEstadosPersonal: { modulo: 'personal', accion: 'ver' },

  listHorarios: { modulo: 'horarios', accion: 'ver' },
  getHorarioGrupo: { modulo: 'horarios', accion: 'ver' },
  createHorario: { modulo: 'horarios', accion: 'crear' },
  updateHorario: { modulo: 'horarios', accion: 'editar' },
  deleteHorario: { modulo: 'horarios', accion: 'eliminar' },
  generarSolicitudHorario: { modulo: 'horarios', accion: 'exportar' },

  listSobretiempo: { modulo: 'sobretiempo', accion: 'ver' },
  getSobretiempo: { modulo: 'sobretiempo', accion: 'ver' },
  obtenerBancoHoras: { modulo: 'sobretiempo', accion: 'ver' },
  listarSolicitudesDescansoBanco: { modulo: 'sobretiempo', accion: 'ver' },
  createSobretiempo: { modulo: 'sobretiempo', accion: 'crear' },
  crearSolicitudDescansoBanco: { modulo: 'sobretiempo', accion: 'crear' },
  updateSobretiempo: { modulo: 'sobretiempo', accion: 'editar' },
  registrarDescansoSobretiempo: { modulo: 'sobretiempo', accion: 'editar' },
  completarDescansoBanco: { modulo: 'sobretiempo', accion: 'editar' },
  deleteSobretiempo: { modulo: 'sobretiempo', accion: 'eliminar' },
  eliminarDescansoSobretiempo: { modulo: 'sobretiempo', accion: 'eliminar' },
  generarSobretiempoXLSX: { modulo: 'sobretiempo', accion: 'exportar' },

  listVacaciones: { modulo: 'vacaciones', accion: 'ver' },
  getVacacion: { modulo: 'vacaciones', accion: 'ver' },
  createVacacion: { modulo: 'vacaciones', accion: 'crear' },
  updateVacacion: { modulo: 'vacaciones', accion: 'editar' },
  registrarGoceVacacion: { modulo: 'vacaciones', accion: 'editar' },
  eliminarGoceVacacion: { modulo: 'vacaciones', accion: 'editar' },
  deleteVacacion: { modulo: 'vacaciones', accion: 'eliminar' },

  listPermisos: { modulo: 'permisos', accion: 'ver' },
  getPermiso: { modulo: 'permisos', accion: 'ver' },
  createPermiso: { modulo: 'permisos', accion: 'crear' },
  updatePermiso: { modulo: 'permisos', accion: 'editar' },
  deletePermiso: { modulo: 'permisos', accion: 'eliminar' },

  listLicencias: { modulo: 'licencias', accion: 'ver' },
  getLicencia: { modulo: 'licencias', accion: 'ver' },
  createLicencia: { modulo: 'licencias', accion: 'crear' },

  listAuditoria: { modulo: 'auditoria', accion: 'ver' },

  getConfigCamposPortal: { modulo: 'portalPersonal', accion: 'ver' },
  setConfigCamposPortal: { modulo: 'portalPersonal', accion: 'editar' }
};

// Objeto de permisos "todo en false", con la forma exacta de
// PERMISOS_MODULOS. Se usa como base al crear un usuario o al leer
// uno guardado con una versión anterior del catálogo (evita que
// falte una clave y rompa tienePermisoAccion_).
function permisosVacios_() {
  const permisos = {};
  Object.keys(PERMISOS_MODULOS).forEach(function(modulo) {
    permisos[modulo] = {};
    PERMISOS_MODULOS[modulo].forEach(function(accion) { permisos[modulo][accion] = false; });
  });
  return permisos;
}

// Sanea lo que llegue del frontend (params.permisos, una cadena
// JSON): solo conserva módulos/acciones que existan en
// PERMISOS_MODULOS y fuerza cada valor a booleano — así una cuenta
// nunca puede terminar con un permiso inventado o mal escrito.
function normalizarPermisos_(permisosCrudos) {
  const base = permisosVacios_();
  let entrada = permisosCrudos;
  if (typeof entrada === 'string') {
    try { entrada = JSON.parse(entrada); } catch (e) { entrada = null; }
  }
  if (!entrada || typeof entrada !== 'object') return base;

  Object.keys(base).forEach(function(modulo) {
    if (entrada[modulo] && typeof entrada[modulo] === 'object') {
      Object.keys(base[modulo]).forEach(function(accion) {
        base[modulo][accion] = entrada[modulo][accion] === true;
      });
    }
  });
  return base;
...
function parsearPermisosGuardados_(valorCelda) {
  if (!valorCelda) return permisosVacios_();
  try {
    return normalizarPermisos_(JSON.parse(valorCelda));
  } catch (e) {
    return permisosVacios_();
  }
}

// Puerta central de permisos, llamada desde handleRequest para TODA
// acción que aparezca en ACCIONES_MODULO. Un "admin" siempre pasa.
// Una acción que no esté mapeada (login, gestión de usuarios, portal
// del personal, etc.) tampoco se bloquea aquí: cada una ya se protege
// por su cuenta (requerirAdmin_, sesión propia del portal, ...).
function tienePermisoAccion_(sesion, action) {
  if (!sesion) return false;
  if (sesion.rol === 'admin') return true;

  const requerido = ACCIONES_MODULO[action];
  if (!requerido) return true; // acción no gobernada por este sistema de permisos

  const permisos = sesion.permisos || {};
  const permisosModulo = permisos[requerido.modulo];
  return !!(permisosModulo && permisosModulo[requerido.accion] === true);
}

// Acciones que no requieren sesión ADMINISTRATIVA iniciada. Además de
// "login" (acceso admin), aquí van las acciones del PORTAL DE
// AUTOACTUALIZACIÓN DEL PERSONAL (ver Codigo_PortalPersonal.gs): ese
// portal usa su propio sistema de sesión (obtenerSesionEmpleado_),
// completamente separado del de BD_USUARIOS, así que cada una de esas
// funciones valida su propio token internamente en vez de depender de
// la sesión admin de más abajo.
const PUBLIC_ACTIONS = ['login', 'loginPersonal', 'logoutPersonal', 'misDatos', 'actualizarMisDatos'];
...
function getUsersSheet() {
  ...
  if (!sheet) {
    ...
    sheet.appendRow([
      ...
      new Date(),
      '',
      '{}' // un admin tiene acceso total; no usa la matriz de permisos
    ]);
  }
...
function crearTokenSesion(usuarioObj) {
  ...
  const payload = JSON.stringify({
    ...
    rol: usuarioObj.rol,
    // Se guarda ya normalizado dentro del propio token de sesión (en
    // CacheService) para que tienePermisoAccion_ pueda comprobarlo en
    // cada petición sin volver a leer BD_USUARIOS.
    permisos: usuarioObj.permisos || permisosVacios_()
  });
...
function login(params) {
  try {
    ...
    const [usuarioGuardado, salt, hashGuardado, nombre, rol, activo, , , permisosGuardados] = encontrado.data;
    ...
    const usuarioObj = {
      ...
      nombre: nombre || usuarioGuardado,
      rol: rol || 'usuario',
      permisos: parsearPermisosGuardados_(permisosGuardados)
    };
...
// ============================================================
// Pantalla "Usuarios" del sidebar (visible solo para ROL = "admin"
// en el frontend): permite crear cuentas nuevas de BD_USUARIOS,
// editar nombre/rol, activar/desactivar el acceso y restablecer la
// contraseña de otra cuenta. handleRequest ya exige sesión válida
// para estas tres acciones (no están en PUBLIC_ACTIONS) y API_KEY
// para crearUsuario/actualizarUsuario (están en WRITE_ACTIONS);
// requerirAdmin_ agrega la comprobación de que, además, esa sesión
// pertenezca a un usuario con ROL = "admin" — sin esto, cualquier
// cuenta normal podría gestionar el resto de usuarios.
// ============================================================

function requerirAdmin_(params) {
  const sesion = obtenerSesion(params.token);
  if (!sesion) return { error: createJsonResponse(false, 'Sesión inválida o expirada. Vuelve a iniciar sesión.') };
  if (sesion.rol !== 'admin') {
    return { error: createJsonResponse(false, 'Solo un administrador puede gestionar usuarios') };
  }
...
// Cuenta cuántos administradores activos quedarían si se excluye al
// usuario indicado — se usa para bloquear el cambio que dejaría al
// sistema sin ningún admin capaz de volver a entrar.
function contarAdminsActivos_(excluirUsuario) {
  const sheet = getUsersSheet();
  const data = sheet.getDataRange().getValues();
  let n = 0;
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const usuario = String(row[USERS_HEADERS.indexOf('USUARIO')]);
    const rol = row[USERS_HEADERS.indexOf('ROL')];
    if (rol === 'admin' && esActivo_(row[USERS_HEADERS.indexOf('ACTIVO')]) &&
        usuario.toLowerCase() !== String(excluirUsuario || '').toLowerCase()) {
      n++;
    }
  }
  return n;
...
// Lista las cuentas de BD_USUARIOS (sin SALT ni PASSWORD_HASH: esos
// dos campos nunca salen del backend).
function listUsuarios(params) {
  try {
    const chk = requerirAdmin_(params);
    if (chk.error) return chk.error;

    const sheet = getUsersSheet();
    const data = sheet.getDataRange().getValues();
    const iUsuario = USERS_HEADERS.indexOf('USUARIO');
    const iNombre = USERS_HEADERS.indexOf('NOMBRE');
    const iRol = USERS_HEADERS.indexOf('ROL');
    const iActivo = USERS_HEADERS.indexOf('ACTIVO');
    const iCreacion = USERS_HEADERS.indexOf('FECHA_CREACION');
    const iAcceso = USERS_HEADERS.indexOf('ULTIMO_ACCESO');
    const iPermisos = USERS_HEADERS.indexOf('PERMISOS');

    const lista = [];
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      lista.push({
        usuario: String(row[iUsuario]),
        nombre: String(row[iNombre] || ''),
        rol: row[iRol] || 'usuario',
        activo: esActivo_(row[iActivo]),
        fechaCreacion: row[iCreacion] instanceof Date ? row[iCreacion].toISOString() : String(row[iCreacion] || ''),
        ultimoAcceso: row[iAcceso] instanceof Date ? row[iAcceso].toISOString() : String(row[iAcceso] || ''),
        permisos: parsearPermisosGuardados_(row[iPermisos])
      });
    }

    return createJsonResponse(true, 'OK', lista);
...
function crearUsuario(params) {
  try {
    ...
    const rol = (params.rol === 'admin') ? 'admin' : 'usuario';
    const activo = params.activo === undefined ? true : esActivo_(params.activo);
    // Un admin no usa permisos granulares (acceso total siempre); para
    // un usuario normal se sanea lo que mande el frontend contra el
    // catálogo de PERMISOS_MODULOS.
    const permisos = (rol === 'admin') ? permisosVacios_() : normalizarPermisos_(params.permisos);

    if (!usuario || !nombre || !password) {
      return createJsonResponse(false, 'Usuario, nombre y contraseña son requeridos');
    }
    if (password.length < 6) {
      return createJsonResponse(false, 'La contraseña debe tener al menos 6 caracteres');
    }
    if (buscarUsuario(usuario)) {
      return createJsonResponse(false, 'Ya existe un usuario con ese nombre de acceso');
    }

    const salt = Utilities.getUuid();
    const sheet = getUsersSheet();
    sheet.appendRow([usuario, salt, hashPassword(password, salt), nombre, rol, activo, new Date(), '', JSON.stringify(permisos)]);

    registrarAuditoria(chk.sesion.usuario, 'CREAR_USUARIO', 'Usuarios',
      'Creó el usuario "' + usuario + '" (' + nombre + ') con rol ' + rol, usuario);

    return createJsonResponse(true, 'Usuario creado correctamente', { usuario: usuario, nombre: nombre, rol: rol, activo: activo, permisos: permisos });
...
function actualizarUsuario(params) {
  try {
    ...
    const usuario = String(params.usuario || '').trim();
    if (!usuario) return createJsonResponse(false, 'Falta indicar el usuario a modificar');
    ...
    const iRol = USERS_HEADERS.indexOf('ROL');
    const iActivo = USERS_HEADERS.indexOf('ACTIVO');
    const iSalt = USERS_HEADERS.indexOf('SALT');
    const iHash = USERS_HEADERS.indexOf('PASSWORD_HASH');
    const iPermisos = USERS_HEADERS.indexOf('PERMISOS');

    const rolActual = encontrado.data[iRol];
    const eraAdminActivo = rolActual === 'admin' && esActivo_(encontrado.data[iActivo]);

    const nombreNuevo = params.nombre !== undefined ? String(params.nombre).trim() : null;
    const rolNuevo = params.rol !== undefined ? (params.rol === 'admin' ? 'admin' : 'usuario') : null;
    const activoNuevo = params.activo !== undefined ? esActivo_(params.activo) : null;
    const passwordNueva = params.passwordNueva !== undefined && params.passwordNueva !== '' ? String(params.passwordNueva) : null;
    // El rol resultante (nuevo si vino en la petición, si no el actual)
    // decide si los permisos que se guardan son la matriz saneada o un
    // objeto vacío (un admin nunca queda con permisos "pegados" de
    // cuando era usuario normal).
    const rolResultante = rolNuevo !== null ? rolNuevo : rolActual;
    let permisosNuevos = null;
    if (rolResultante === 'admin' && rolNuevo !== null) {
      // Pasó a (o se confirma como) admin: nunca debe quedar con
      // permisos de una etapa anterior como usuario normal.
      permisosNuevos = permisosVacios_();
    } else if (params.permisos !== undefined) {
      permisosNuevos = (rolResultante === 'admin') ? permisosVacios_() : normalizarPermisos_(params.permisos);
    }

    if (nombreNuevo !== null && !nombreNuevo) {
      return createJsonResponse(false, 'El nombre no puede quedar vacío');
    }
    if (passwordNueva !== null && passwordNueva.length < 6) {
      return createJsonResponse(false, 'La nueva contraseña debe tener al menos 6 caracteres');
    }

    // Salvaguarda: no dejar al sistema sin ningún administrador
    // activo (ni quitándole el rol, ni desactivando la cuenta).
    const dejariaDeSerAdminActivo = eraAdminActivo &&
      ((rolNuevo !== null && rolNuevo !== 'admin') || activoNuevo === false);
    if (dejariaDeSerAdminActivo && contarAdminsActivos_(usuario) === 0) {
      return createJsonResponse(false, 'No puedes hacer ese cambio: "' + usuario + '" es el único administrador activo del sistema');
    }

    const sheet = getUsersSheet();
    if (nombreNuevo !== null) sheet.getRange(encontrado.rowIndex, iNombre + 1).setValue(nombreNuevo);
    if (rolNuevo !== null) sheet.getRange(encontrado.rowIndex, iRol + 1).setValue(rolNuevo);
    if (activoNuevo !== null) sheet.getRange(encontrado.rowIndex, iActivo + 1).setValue(activoNuevo);
    if (permisosNuevos !== null) sheet.getRange(encontrado.rowIndex, iPermisos + 1).setValue(JSON.stringify(permisosNuevos));
    if (passwordNueva !== null) {
      const nuevoSalt = Utilities.getUuid();
      sheet.getRange(encontrado.rowIndex, iSalt + 1).setValue(nuevoSalt);
      sheet.getRange(encontrado.rowIndex, iHash + 1).setValue(hashPassword(passwordNueva, nuevoSalt));
    }

    registrarAuditoria(chk.sesion.usuario, 'ACTUALIZAR_USUARIO', 'Usuarios',
      'Actualizó el usuario "' + usuario + '"'
      + (passwordNueva !== null ? ' (incluye restablecer contraseña)' : '')
      + (permisosNuevos !== null ? ' (permisos de acceso actualizados)' : ''), usuario);

    return createJsonResponse(true, 'Usuario actualizado correctamente');
...
function handleRequest(e) {
  ...
  if (PUBLIC_ACTIONS.indexOf(action) === -1) {
    ...
    // Permisos de acceso granulares (checkboxes por módulo/acción,
    // ver PERMISOS_MODULOS / ACCESOS_MODULO arriba): esta es la
    // comprobación REAL, del lado servidor — ocultar un botón en el
    // sidebar (ver app.js) es solo comodidad visual. Un admin siempre
    // pasa; una cuenta normal necesita el permiso exacto que exige
    // esta acción.
    if (!tienePermisoAccion_(sesion, action)) {
      return createJsonResponse(false, 'No tienes permiso para realizar esta acción. Contacta a un administrador.');
    }
  }
  ...
  switch (action) {
    ...
    case 'eliminarDescansoSobretiempo':
      return eliminarDescansoSobretiempo(params);
    ...
    case 'obtenerBancoHoras':
      return obtenerBancoHoras(params);
    case 'crearSolicitudDescansoBanco':
      return crearSolicitudDescansoBanco(params);
    case 'listarSolicitudesDescansoBanco':
      return listarSolicitudesDescansoBanco(params);
   ...
   case 'getPermiso':
    ...
    // ---- Módulo Vacaciones (ver Codigo_Vacaciones.gs) ----
    // Mismo patrón de dos fases que Sobretiempo: Fase 1 registra el
    // saldo del período (createVacacion/updateVacacion/deleteVacacion,
    // listVacaciones/getVacacion para leer) y Fase 2 agrega/quita
    // tramos de goce que descuentan ese saldo (registrarGoceVacacion/
    // eliminarGoceVacacion). No hay generarVacacionXLSX: el reporte
    // se arma enteramente en el navegador a partir de listVacaciones
    // (ver js/vacaciones/vacaciones-export-xlsx.js).
    case 'createVacacion':
      return createVacacion(params);
    case 'updateVacacion':
      return updateVacacion(params);
    case 'deleteVacacion':
      return deleteVacacion(params);
    case 'listVacaciones':
      return listVacaciones(params);
    case 'getVacacion':
      return getVacacion(params);
    case 'registrarGoceVacacion':
      return registrarGoceVacacion(params);
    case 'eliminarGoceVacacion':
      return eliminarGoceVacacion(params);
    // ---- Módulo Licencias (ver Codigo_Licencias.gs) ----
    case 'createLicencia':
      return createLicencia(params);
    case 'listLicencias':
      return listLicencias(params);
    case 'getLicencia':
      return getLicencia(params);

    // ---- PORTAL DE AUTOACTUALIZACIÓN DEL PERSONAL (ver Codigo_PortalPersonal.gs) ----
    // loginPersonal/logoutPersonal/misDatos/actualizarMisDatos usan su
    // propia sesión (obtenerSesionEmpleado_), separada de la sesión
    // admin — por eso están en PUBLIC_ACTIONS y cada función valida su
    // propio token por dentro.
    case 'loginPersonal':
      return loginPersonal(params);
    case 'logoutPersonal':
      return logoutPersonal(params);
    case 'misDatos':
      return misDatosPersonal(params);
    case 'actualizarMisDatos':
      return actualizarMisDatosPersonal(params);
    case 'listEstadosPersonal':
      return listEstadosPersonal(params);
    // Configuración (editable/visible por campo) del Portal de
    // Autoactualización — acciones ADMINISTRATIVAS: exigen sesión
    // admin normal (no están en PUBLIC_ACTIONS). La de guardar además
    // exige la API key por estar en WRITE_ACTIONS.
    case 'getConfigCamposPortal':
      return getConfigCamposPortal(params);
    case 'setConfigCamposPortal':
      return setConfigCamposPortal(params);

    default:
      return createJsonResponse(false, 'Acción no reconocida. Acciones disponibles: login, logout, cambiarPassword, listUsuarios, crearUsuario, actualizarUsuario, listAuditoria, list, create, update, delete, createHorario, updateHorario, deleteHorario, listHorarios, getHorarioGrupo, generarSolicitudHorario, createSobretiempo, updateSobretiempo, registrarDescansoSobretiempo, deleteSobretiempo, listSobretiempo, getSobretiempo, createPermiso, updatePermiso, deletePermiso, listPermisos, getPermiso, createVacacion, updateVacacion, deleteVacacion, listVacaciones, getVacacion, registrarGoceVacacion, eliminarGoceVacacion, createLicencia, listLicencias, getLicencia, loginPersonal, logoutPersonal, misDatos, actualizarMisDatos, listEstadosPersonal, getConfigCamposPortal, setConfigCamposPortal');
  }
}
...
// Junta las OBSERVACIONES cargadas por día en el módulo Horarios
// (mismo campo "obs" que se ve en el modal/página web, columna P de
// BD_HORARIOS) en un solo texto para la celda A19 de la Solicitud.
// Si todos los días con observación comparten exactamente el mismo
// texto (caso más común: una sola observación aplicada a todo el
// horario), se muestra una sola vez; si varían día a día, se listan
// una por línea con el día delante.
function observacionesGrupo_(grupo) {
  if (!grupo) return '';
  const conObs = (grupo.DIAS || []).filter(d => d.obs);
  if (conObs.length === 0) return '';
  const textos = conObs.map(d => d.obs);
  const todasIguales = textos.every(t => t === textos[0]);
  if (todasIguales) return textos[0];
  return conObs.map(d => `${d.dia}: ${d.obs}`).join('\n');
}
...
function generarSolicitudHorario(params) {
  try {
    ...
    // Ya NO se elige por ESTADO (Vigente/Programado): ese campo se
    // calcula solo a partir de fechas y no siempre coincide con cuál
    // es el cambio que se está tramitando en ESTA solicitud. En su
    // lugar se ordenan TODOS los horarios del empleado por orden de
    // REGISTRO en el sistema (FECHA_REGISTRO; si faltara, se usa
    // FECHA_INICIO como respaldo) y se toman los dos últimos:
    //   - el ÚLTIMO registrado    → HORARIO PROPUESTO (B17/D14/F14):
    //     es el horario nuevo que motiva la solicitud de cambio.
    //   - el PENÚLTIMO registrado → HORARIO VIGENTE (B10): el que
    //     rige justo antes de ese cambio.
    const gruposOrdenados = grupos.slice().sort((a, b) => {
      const fa = new Date(a.FECHA_REGISTRO || a.FECHA_INICIO);
      const fb = new Date(b.FECHA_REGISTRO || b.FECHA_INICIO);
      return fa - fb;
    });
    const propuesto = gruposOrdenados[gruposOrdenados.length - 1] || null;
    const vigente = gruposOrdenados.length > 1 ? gruposOrdenados[gruposOrdenados.length - 2] : null;
    ...
    if (propuesto) {
      ...
      // FECHA_FIN: si el horario propuesto no tiene fecha de término
      // registrada, la celda dice "A la actualidad" en vez de quedar
      // vacía (mismo texto que ya usa el detalle .xlsx del frontend
      // para este mismo caso).
      hoja.getRange(CELDAS_SOLICITUD_HORARIO.FECHA_FIN)
        .setValue(propuesto.FECHA_FIN || 'A la actualidad');
      hoja.getRange(CELDAS_SOLICITUD_HORARIO.HORARIO_PROPUESTO).setValue(textoHorarioGrupo_(propuesto));
      hoja.getRange(CELDAS_SOLICITUD_HORARIO.OBSERVACIONES).setValue(observacionesGrupo_(propuesto));
    }
    // MARCAR CON X (Temporal/Permanente/Rotativo) y MOTIVO quedan en
    // blanco a propósito: son decisiones humanas, no datos que el
    // sistema pueda inferir.
...
function testVacacionesFlujoCompleto() {
  const token = testLoginYObtenerToken();

  const creado = JSON.parse(doGet({ parameter: {
    action: 'createVacacion', key: API_KEY, token: token,
    CODE: 'P1', ID_PERSONAL: '12345', EMPLEADO: 'Pérez García, Juan',
    PERIODO_VACACIONAL: '2025-2026', DIAS_ASIGNADOS: '30', FECHA_LIMITE: '2026-12-31'
  } }).getContent());
  Logger.log(creado);
  const idVacacion = creado.data.ID_VACACION;

  const goce = JSON.parse(doGet({ parameter: {
    action: 'registrarGoceVacacion', key: API_KEY, token: token,
    ID_VACACION: idVacacion, FECHA_INICIO: '2025-12-10', FECHA_FIN: '2025-12-24', DIAS_TOMADOS: '15'
  } }).getContent());
  Logger.log(goce);

  const lista = JSON.parse(doGet({ parameter: { action: 'listVacaciones', token: token, code: 'P1' } }).getContent());
  Logger.log(lista);
}