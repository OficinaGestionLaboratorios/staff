...
function loginPersonal(params) {
  try {
    const idPersonal = String(params.idPersonal || params.usuario || '').trim();
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (String(row[1]).trim() === idPersonal && String(row[7]).trim() === dni) {
        encontrado = row;
        break;
      }
    }
    ...
    if (!encontrado) {
      registrarAuditoria(idPersonal, 'LOGIN_FALLIDO', 'PortalPersonal', 'ID o DNI incorrectos');
      return createJsonResponse(false, 'Usuario o clave incorrectos.');
    }

    const code = encontrado[0];
    const nombreCompleto = `${encontrado[4] || ''} ${encontrado[2] || ''} ${encontrado[3] || ''}`.trim();
    const token = crearTokenSesionEmpleado_(code, idPersonal);

    registrarAuditoria(idPersonal, 'LOGIN', 'PortalPersonal', `Ingreso al portal de autoactualización (${code})`, code);
...
function misDatosPersonal(params) {
  try {
    ...
    if (!resultado || String(resultado.data[1]).trim() !== String(sesion.idPersonal).trim()) {
