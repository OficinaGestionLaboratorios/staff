...
// ============================================================
// ★★★ CORRECCIÓN (revisión de lógica) ★★★
// 1) registrarDescansoEfectuadoBanco: el segundo (o siguiente) abono
//    de descanso de una misma solicitud fallaba casi siempre porque
//    se revalidaba contra la SELECCIÓN ORIGINAL completa en vez de
//    contra lo que realmente falta por consumir. Corregido: ver
//    comentario "---- CORRECCIÓN ----" dentro de esa función.
// 2) crearSolicitudDescansoBanco: se agregó LockService (igual que ya
//    tenía registrarDescansoEfectuadoBanco) para evitar que dos
//    solicitudes casi simultáneas sobre-reserven el mismo saldo.
// Reemplaza TODO tu archivo Codigo_Sobretiempo.gs por este contenido
// y vuelve a publicar (no se requieren otros cambios en
// Codigo_corregido.gs: las acciones del switch/WRITE_ACTIONS ya
// estaban registradas).
// ============================================================
...
function solicitudTieneMovimientosBanco_(idSolicitud) {
  try {
    return leerMovimientosBanco_().some(m =>
      String(m.ID_SOLICITUD_ORIGEN) === String(idSolicitud) && m.ESTADO === 'UTILIZADO'
    );
  } catch(e) { return false; }
}

// Una solicitud pendiente de banco no descuenta saldo, pero sí debe
// proteger la solicitud de Fase 1 de cambios/eliminación que dejen
// inválida la referencia utilizada por el pedido pendiente.
function solicitudTieneSolicitudBancoPendiente_(idSolicitud) {
  try {
    const sh = getDescansoBancoSheet_();
    const data = sh.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) {
      if (!data[i][0] || String(data[i][9] || '') !== 'PENDIENTE') continue;
      let selecciones = [];
      try { selecciones = JSON.parse(data[i][5] || '[]'); } catch(e) { selecciones = []; }
      if (selecciones.some(s => String(s.ID_SOLICITUD_ORIGEN) === String(idSolicitud))) return true;
    }
  } catch(e) {}
  return false;
}
...
function updateSobretiempo(params) {
  try {
    ...
    if (solicitudTieneMovimientosBanco_(id)) {
      return createJsonResponse(false, 'No se puede editar: esta solicitud ya tiene horas utilizadas en el banco de horas');
    }
    if (solicitudTieneSolicitudBancoPendiente_(id)) {
      return createJsonResponse(false, 'No se puede editar: esta solicitud tiene un descanso compensatorio pendiente asociado al banco de horas');
    }
...
function deleteSobretiempo(params) {
  try {
    ...
    if (solicitudTieneMovimientosBanco_(id)) return createJsonResponse(false, 'No se puede eliminar: la solicitud tiene horas utilizadas en el banco de horas');
    if (solicitudTieneSolicitudBancoPendiente_(id)) return createJsonResponse(false, 'No se puede eliminar: la solicitud tiene un descanso compensatorio pendiente asociado al banco de horas');
...
function armarObjetoSobretiempo_(row, movimientosBanco) {
  ...
  // Solo los consumos reales UTILIZADOS del banco cuentan como
  // horas efectivamente compensadas. Las solicitudes PENDIENTES no
  // generan movimiento de saldo y los movimientos históricos
  // Los movimientos históricos de reserva se conservan únicamente por trazabilidad.
  ...
  const obj = {
    ...
    TOTAL_HORAS_EFECTIVAS: totalHorasEfectivas, // descansos viejos + consumos reales del banco
  ...
  try {
    ...
    registros.forEach(r=>{
      ...
      (r.FECHAS||[]).forEach(f=>{
        ...
        const disponibles=Math.max(generadas-utilizadas,0);
        banco.push({
          ...
          HORAS_DISPONIBLES:disponibles.toFixed(2),
          ESTADO: disponibles>0 ? 'Disponible' : 'Agotada' 
        });
      });
    });
...
function obtenerHorasPendientesPorFuenteBanco_(code, excluirSolicitudId) {
  const out={};
  try {
    const sh=getDescansoBancoSheet_();
    const data=sh.getDataRange().getValues();
    for(let i=1;i<data.length;i++) {
      if(!data[i][0] || String(data[i][2])!==String(code) || String(data[i][9]||'')!=='PENDIENTE') continue;
      if (excluirSolicitudId && String(data[i][0]) === String(excluirSolicitudId)) continue;
      let selecciones=[];
      try { selecciones=JSON.parse(data[i][5]||'[]'); } catch(e) { selecciones=[]; }
      selecciones.forEach(s=>{
        const key=String(s.ID_SOLICITUD_ORIGEN)+'|'+String(s.FECHA_SOBRETIEMPO);
        out[key]=(out[key]||0)+(parseFloat(s.HORAS_A_UTILIZAR)||0);
      });
    }
  } catch(e) {}
  return out;
...
function validarSeleccionesBanco_(code, selecciones, excluirSolicitudId) {
  ...
  const pendientes=obtenerHorasPendientesPorFuenteBanco_(code, excluirSolicitudId);
  const nuevas={};
  let total=0;
  for(const s of selecciones) {
    ...
    nuevas[key]=(nuevas[key]||0)+h;
    const yaSolicitadas=pendientes[key]||0;
    const disponible=parseFloat(f.HORAS_DISPONIBLES)||0;
    if(yaSolicitadas+nuevas[key]-disponible>0.001) {
      return {error:`Las horas solicitadas para ${s.FECHA_SOBRETIEMPO} ya tienen ${yaSolicitadas.toFixed(2)} h en una solicitud pendiente. Saldo disponible real: ${disponible.toFixed(2)} h.`};
    }
    total+=h;
  }
...
// CORRECCIÓN: se agrega LockService, igual que en
// registrarDescansoEfectuadoBanco. Antes esta función validaba y
// escribía sin lock: dos solicitudes casi simultáneas que usan el
// mismo rango de horas de una misma fecha de sobretiempo podían pasar
// la validación juntas y sobre-reservar el mismo saldo.
function crearSolicitudDescansoBanco(params) {
  ...
  try {
    ...
    // Una solicitud PENDIENTE no genera movimiento de saldo.
    // El consumo real se registrará posteriormente como UTILIZADO
    // cuando el descanso haya sido efectivamente confirmado.
...
function anularSolicitudDescansoBanco(params) {
  try {
    const id = String(params.ID_SOLICITUD_DESCANSO || params.idSolicitudDescanso || '').trim();
    if (!id) return createJsonResponse(false,'ID_SOLICITUD_DESCANSO es requerido');
    const sh=getDescansoBancoSheet_();
    const data=sh.getDataRange().getValues();
    const H=DESCANSO_BANCO_HEADERS;
    for(let i=1;i<data.length;i++) {
      if(String(data[i][0])!==id) continue;
      if(String(data[i][9]||'')!=='PENDIENTE') return createJsonResponse(false,'Solo se pueden anular solicitudes que estén PENDIENTE');
      sh.getRange(i+1,H.indexOf('ESTADO')+1).setValue('ANULADA');
      registrarAuditoria(params.__usuario,'ANULAR','Descanso compensatorio',`Solicitud ${id} anulada`,id);
      return createJsonResponse(true,'Solicitud anulada correctamente',{ID_SOLICITUD_DESCANSO:id,ESTADO:'ANULADA'});
    }
    return createJsonResponse(false,'No se encontró la solicitud de descanso');
  } catch(e) { return createJsonResponse(false,e.toString()); }
}


// ============================================================
// REGISTRO DE DESCANSO EFECTUADO DESDE EL BANCO DE HORAS
// ============================================================
// Una solicitud de BD_DESCANSO_COMPENSATORIO en estado PENDIENTE
// no consume saldo. Esta función es el único punto que convierte
// la solicitud en consumo real: crea movimientos UTILIZADO y luego
// marca la solicitud como UTILIZADO.
// FECHAS_DESCANSO_EFECTUADO_JSON: [{fecha, horas}]
function registrarDescansoEfectuadoBanco(params) {
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(15000);
    const id = String(params.ID_SOLICITUD_DESCANSO || params.idSolicitudDescanso || '').trim();
    ...
    let fechasEfectuado = [];
    try { fechasEfectuado = JSON.parse(params.FECHAS_DESCANSO_EFECTUADO_JSON || '[]'); }
    catch (e) { return createJsonResponse(false, 'FECHAS_DESCANSO_EFECTUADO_JSON inválido'); }
    if (!Array.isArray(fechasEfectuado) || !fechasEfectuado.length) {
      return createJsonResponse(false, 'Debe registrar al menos una fecha efectiva de descanso');
    }

    const descansoSh = getDescansoBancoSheet_();
    const descansoData = descansoSh.getDataRange().getValues();
    const H = DESCANSO_BANCO_HEADERS;
    let fila = -1, solicitud = null;
    for (let i = 1; i < descansoData.length; i++) {
      if (String(descansoData[i][0] || '').trim() === id) {
        fila = i + 1;
        let selecciones = [];
        try { selecciones = JSON.parse(descansoData[i][5] || '[]'); } catch (e) { selecciones = []; }
        solicitud = {
          ID_SOLICITUD_DESCANSO: String(descansoData[i][0] || ''),
          CODE: String(descansoData[i][2] || ''),
          ID_PERSONAL: String(descansoData[i][3] || ''),
          EMPLEADO: String(descansoData[i][4] || ''),
          SELECCIONES: selecciones,
          TOTAL_HORAS: normalizarHorasDecimal_(descansoData[i][7]),
          ESTADO: String(descansoData[i][9] || '')
        };
        break;
      }
    }
    if (!solicitud) return createJsonResponse(false, 'No se encontró la solicitud de descanso');
    if (['PENDIENTE', 'PARCIALMENTE UTILIZADO'].indexOf(solicitud.ESTADO) === -1) {
      return createJsonResponse(false, `La solicitud ${id} no permite registrar más consumo. Estado actual: ${solicitud.ESTADO || 'sin estado'}`);
    }

    const hoy = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
    const fechas = fechasEfectuado.map(f => ({
      fecha: String(f?.fecha || '').slice(0, 10),
      horas: parseFloat(f?.horas) || 0
    })).filter(f => f.fecha && f.horas > 0);
    if (!fechas.length) return createJsonResponse(false, 'Las fechas efectivas deben tener fecha y horas mayores que cero');
    const fechaFutura = fechas.find(f => f.fecha > hoy);
    if (fechaFutura) return createJsonResponse(false, `La fecha efectiva ${fechaFutura.fecha} no puede ser futura.`);

    const totalEfectuado = fechas.reduce((a, f) => a + f.horas, 0);
    const movimientos = leerMovimientosBanco_();
    const consumosPrevios = movimientos.filter(m => String(m.ID_SOLICITUD_DESCANSO) === id && String(m.ESTADO) === 'UTILIZADO');
    const totalPrevio = consumosPrevios.reduce((a, m) => a + (parseFloat(m.HORAS) || 0), 0);
    const restanteSolicitud = Math.max(solicitud.TOTAL_HORAS - totalPrevio, 0);

    if (restanteSolicitud <= 0.001) {
      return createJsonResponse(false, `La solicitud ${id} ya está completamente utilizada.`);
    }
    if (totalEfectuado - restanteSolicitud > 0.01) {
      return createJsonResponse(false, `Solo quedan ${restanteSolicitud.toFixed(2)} h por utilizar de esta solicitud.`);
    }

    // Consumo acumulado de ESTA solicitud por cada fuente de sobretiempo
    // (se calcula ANTES de revalidar, porque la revalidación de abajo
    // debe comparar contra lo que aún FALTA, no contra la selección
    // original completa — ver nota de la corrección más abajo).
    const usadoPorFuente = {};
    consumosPrevios.forEach(m => {
      const key = String(m.ID_SOLICITUD_ORIGEN) + '|' + String(m.FECHA_SOBRETIEMPO);
      usadoPorFuente[key] = (usadoPorFuente[key] || 0) + (parseFloat(m.HORAS) || 0);
    });

    // Determinar cuánto queda de cada fuente dentro de la selección original.
    const fuentesRestantes = (solicitud.SELECCIONES || []).map(s => {
      const key = String(s.ID_SOLICITUD_ORIGEN) + '|' + String(s.FECHA_SOBRETIEMPO);
      const solicitadoFuente = parseFloat(s.HORAS_A_UTILIZAR) || 0;
      const usadoFuente = usadoPorFuente[key] || 0;
      return { s, key, restante: Math.max(solicitadoFuente - usadoFuente, 0) };
    }).filter(x => x.restante > 0.001);

    const totalFuentesRestantes = fuentesRestantes.reduce((a, x) => a + x.restante, 0);
    if (totalEfectuado - totalFuentesRestantes > 0.01) {
      return createJsonResponse(false, `Las horas pendientes de esta solicitud son ${totalFuentesRestantes.toFixed(2)} h y no alcanzan para registrar ${totalEfectuado.toFixed(2)} h.`);
    }

    // ---- CORRECCIÓN ----
    // Antes se revalidaba contra `solicitud.SELECCIONES` (la selección
    // ORIGINAL completa), pero el saldo disponible del banco YA está
    // descontado por lo que esta misma solicitud consumió en abonos
    // anteriores. Eso hacía que, desde el segundo registro de descanso
    // en adelante, la validación comparara "5 h originales" contra
    // "2 h realmente disponibles" (porque ya se usaron 3 h antes) y
    // rechazara el registro aunque nadie más hubiera tomado esas horas.
    // Ahora se revalida solo contra lo que REALMENTE falta por consumir
    // de esta solicitud (fuentesRestantes), que es lo correcto: el saldo
    // del banco ya excluye el consumo propio, así que hay que compararlo
    // contra la necesidad restante, no contra el total original.
    const seleccionesRestantes = fuentesRestantes.map(x => ({
      ID_SOLICITUD_ORIGEN: x.s.ID_SOLICITUD_ORIGEN,
      FECHA_SOBRETIEMPO: x.s.FECHA_SOBRETIEMPO,
      HORAS_A_UTILIZAR: x.restante
    }));
    const validacion = validarSeleccionesBanco_(solicitud.CODE, seleccionesRestantes, id);
    if (validacion.error) return createJsonResponse(false, validacion.error);
    const banco = {};
    (validacion.banco || []).forEach(x => {
      banco[String(x.ID_SOLICITUD_ORIGEN) + '|' + String(x.FECHA_SOBRETIEMPO)] = x;
    });

    const movimientosAInsertar = [];
    let fuenteIdx = 0;
    let restanteFuente = fuentesRestantes.length ? fuentesRestantes[0].restante : 0;
    let totalPorInsertar = 0;

    // Distribuye SOLO el nuevo consumo entre las horas que aún quedan
    // de la selección original. Permite varias registraciones parciales.
    for (const fecha of fechas) {
      let restanteFecha = fecha.horas;
      while (restanteFecha > 0.001) {
        while (fuenteIdx < fuentesRestantes.length && restanteFuente <= 0.001) {
          fuenteIdx++;
          if (fuenteIdx < fuentesRestantes.length) restanteFuente = fuentesRestantes[fuenteIdx].restante;
        }
        if (fuenteIdx >= fuentesRestantes.length) return createJsonResponse(false, 'No quedan horas de la solicitud por consumir.');

        const item = fuentesRestantes[fuenteIdx];
        const fuente = banco[item.key];
        if (!fuente) return createJsonResponse(false, `No se pudo validar la fuente de sobretiempo ${item.s.FECHA_SOBRETIEMPO}`);
        const disponibleFuente = parseFloat(fuente.HORAS_DISPONIBLES) || 0;
        const tomar = Math.min(restanteFecha, restanteFuente, disponibleFuente);
        if (tomar <= 0.001) return createJsonResponse(false, `El saldo disponible de ${item.s.FECHA_SOBRETIEMPO} ya no es suficiente.`);

        movimientosAInsertar.push([
          'MB-' + Utilities.getUuid().replace(/-/g, '').slice(0, 12).toUpperCase(),
          new Date(), id, solicitud.CODE, solicitud.ID_PERSONAL,
          item.s.ID_SOLICITUD_ORIGEN || '', item.s.FECHA_SOBRETIEMPO || '',
          Number(tomar.toFixed(2)), 'CONSUMO', 'UTILIZADO', fecha.fecha,
          String(params.OBSERVACIONES || `Descanso efectuado de la solicitud ${id}`).trim(),
          params.__usuario || ''
        ]);
        totalPorInsertar += tomar;
        restanteFecha -= tomar;
        restanteFuente -= tomar;
      }
    }

    if (Math.abs(totalPorInsertar - totalEfectuado) > 0.01) {
      return createJsonResponse(false, 'No fue posible registrar exactamente las horas indicadas.');
    }

    const movSh = getBancoMovimientosSheet_();
    if (movimientosAInsertar.length) {
      movSh.getRange(movSh.getLastRow() + 1, 1, movimientosAInsertar.length, BANCO_SOBRETIEMPO_HEADERS.length).setValues(movimientosAInsertar);
    }

    const totalAcumulado = totalPrevio + totalEfectuado;
    const nuevoEstado = totalAcumulado + 0.01 >= solicitud.TOTAL_HORAS ? 'COMPLETADO' : 'PARCIALMENTE UTILIZADO';
    descansoSh.getRange(fila, H.indexOf('ESTADO') + 1).setValue(nuevoEstado);
    const observacionActual = String(descansoData[fila - 1][H.indexOf('OBSERVACIONES')] || '').trim();
    const resumenEfectivo = fechas.map(f => `${f.fecha}: ${f.horas.toFixed(2)} h`).join(' | ');
    const observacionFinal = [observacionActual, `DESCANSO EFECTUADO (${nuevoEstado}): ${resumenEfectivo}`].filter(Boolean).join('\n');
    descansoSh.getRange(fila, H.indexOf('OBSERVACIONES') + 1).setValue(observacionFinal);

    registrarAuditoria(params.__usuario, 'UTILIZAR', 'Descanso compensatorio', `Solicitud ${id}: ${nuevoEstado}; consumo acumulado ${totalAcumulado.toFixed(2)} de ${solicitud.TOTAL_HORAS.toFixed(2)} h`, id);
    return createJsonResponse(true, 'Descanso efectuado registrado correctamente', {
      ID_SOLICITUD_DESCANSO: id,
      TOTAL_HORAS: solicitud.TOTAL_HORAS.toFixed(2),
      HORAS_REGISTRADAS_AHORA: totalEfectuado.toFixed(2),
      HORAS_UTILIZADAS_ACUMULADAS: totalAcumulado.toFixed(2),
      HORAS_PENDIENTES: Math.max(solicitud.TOTAL_HORAS - totalAcumulado, 0).toFixed(2),
      ESTADO: nuevoEstado,
      FECHAS_EFECTIVAS: fechas
    });
  } catch (e) {
    return createJsonResponse(false, e.toString());
  } finally {
...
function listarSolicitudesDescansoBanco(params) {
  try {
    ...
    for(let i=1;i<data.length;i++) {
      ...
      const idSolicitud = String(data[i][0] || '');
      const movimientos = leerMovimientosBanco_().filter(m => String(m.ID_SOLICITUD_DESCANSO) === idSolicitud && String(m.ESTADO) === 'UTILIZADO');
      const horasUtilizadas = movimientos.reduce((a,m)=>a+(parseFloat(m.HORAS)||0),0);
      const horasPendientes = Math.max(parseFloat(totalHoras)-horasUtilizadas,0);
      let estado = String(data[i][9] || 'PENDIENTE');
      // Para el nuevo flujo solo se presentan estos tres estados.
      if (horasUtilizadas >= parseFloat(totalHoras)-0.01) estado = 'COMPLETADO';
      else if (horasUtilizadas > 0.001) estado = 'PARCIALMENTE UTILIZADO';
      else estado = 'PENDIENTE';
      out.push({ID_SOLICITUD_DESCANSO:idSolicitud,FECHA_REGISTRO:data[i][1] instanceof Date?data[i][1].toISOString():data[i][1],CODE:data[i][2],ID_PERSONAL:data[i][3],EMPLEADO:data[i][4],SELECCIONES:selecciones,FECHAS_DESCANSO:JSON.parse(data[i][6]||'[]'),TOTAL_HORAS:totalHoras,HORAS_UTILIZADAS:horasUtilizadas.toFixed(2),HORAS_PENDIENTES:horasPendientes.toFixed(2),EQUIVALENCIA:data[i][8],ESTADO:estado,OBSERVACIONES:data[i][10]});
    }
